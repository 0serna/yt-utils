...
function readSpeedLanguage(snapshot: PlayerSnapshot): string | null {
  if (snapshot.audioLanguage) {
    return snapshot.audioLanguage;
  }

  if (snapshot.contentLanguage) {
    return snapshot.contentLanguage;
  }

  return readAutoGeneratedCaptionLanguage(snapshot);
}

function readAutoGeneratedCaptionLanguage(
  snapshot: PlayerSnapshot,
): string | null {
  return (
    findAutoGeneratedCaption(snapshot, isEnglishLanguage) ||
    findAutoGeneratedCaption(snapshot, isSpanishLanguage)
  );
}

function findAutoGeneratedCaption(
  snapshot: PlayerSnapshot,
  isTargetLanguage: (value: string | null | undefined) => boolean,
): string | null {
  const track = snapshot.captionTracks.find(
    (candidate) =>
      isTargetLanguage(candidate.languageCode) &&
      isAutoGeneratedCaptionTrack(candidate),
  );
  return track?.languageCode ?? null;
}

function isAutoGeneratedCaptionTrack(
  track: PlayerSnapshot["captionTracks"][number],
): boolean {
  return (
...
function getSpeedForLanguage(audioLanguage: string | null | undefined): number {
  ...
  if (isEnglishLanguage(audioLanguage)) {
    return normalizePlaybackSpeed(1);
  }
