...
function findInsertionTarget(actionsContainer: HTMLElement): HTMLElement | null {
  const candidates = [...actionsContainer.children].filter(
    (child) => child.id !== CONTROL_HOST_ID && child.id !== MARK_AS_SEEN_HOST_ID,
  ) as HTMLElement[];
...
function syncControlState(): void {
  const decrementBtn = document.getElementById(SPEED_DECREMENT_ID) as HTMLButtonElement | null;
  const incrementBtn = document.getElementById(SPEED_INCREMENT_ID) as HTMLButtonElement | null;
  ...
  observer = new MutationObserver((mutations) => {
    const hasRelevantMutation = mutations.some((mutation) => {
      ...
      return [...mutation.addedNodes, ...mutation.removedNodes].some((node) => {
        ...
        if (node.id === CONTROL_HOST_ID || node.querySelector?.(`#${CONTROL_HOST_ID}`)) {
          return false;
        }
...
function isInsideSpeedControl(node: Node): boolean {
  return node instanceof Element && Boolean(node.closest(`#${CONTROL_HOST_ID}`));
}
