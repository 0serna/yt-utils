...
import { runMarkAsSeenForTab, clearActionStatus } from "./automation";
...
export function registerMarkAsSeenHandler(): void {
  onMessage(
    (message) => (message as { type?: string })?.type === MESSAGE_INLINE_TRIGGER,
    async (_message, sender) => {
      ...
      try {
        ...
        const result = await runMarkAsSeenForTab(tabId, tab.url, { redirectTab: true });
        ...
        const messageText = error instanceof Error ? error.message : "The automation failed.";
