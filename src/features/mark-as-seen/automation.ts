...
function createExtensionError(code: string, message: string, details?: unknown): Error & { code: string; details: unknown } {
...
function runYoutubeMarkAsSeenAutomation(): Promise<AutomationResult> {
  ...
  async function waitFor<T>(
    ...
    options?: { timeout?: number; interval?: number; errorCode?: string; errorMessage?: string },
  ...
  function findButton(root: ParentNode, matchers: readonly RegExp[]): HTMLElement | null {
    const elements = root.querySelectorAll<HTMLElement>("button, [role='button'], tp-yt-paper-checkbox[role='checkbox']");

    return [...elements].find((element) => isVisible(element) && matchesAnyLabel(element, matchers)) ?? null;
  }

  function findShareDialog(): HTMLElement | null {
    const dialogs = document.querySelectorAll<HTMLElement>("tp-yt-paper-dialog, [role='dialog']");
  ...
  function findStartAtCheckbox(dialog: ParentNode, matchers: readonly RegExp[]): HTMLElement | null {
    const selectors = [
      "#start-at-checkbox",
  ...
  async function execute(): Promise<AutomationResult> {
    try {
      ...
      const video = await waitFor(() => document.querySelector<HTMLVideoElement>("video"), {
        timeout: 15000,
      ...
      return {
        ...
        message: (error as Error)?.message || "The automation did not complete successfully.",
...
export { runYoutubeMarkAsSeenAutomation, clearActionStatus };

export async function runMarkAsSeenForTab(
  tabId: number,
  rawUrl: string | undefined,
  options: { redirectTab?: boolean } = {},
): Promise<{ ok: boolean; redirectUrl?: string; code?: string; message: string; details?: unknown }> {
  try {
    ...
    const message = error instanceof Error ? error.message : "The automation failed.";
