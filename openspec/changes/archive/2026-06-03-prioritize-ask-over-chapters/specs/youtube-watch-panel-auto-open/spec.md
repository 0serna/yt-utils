...
The system MUST automatically decide and open the best available YouTube engagement panel for the current confirmed watch-page video. The system MUST treat a changed watch-page URL `v` parameter as a new panel-auto-open session, invalidate pending work from the previous video, and only act when the live player confirms the same current video ID. The system MUST prioritize Ask/Preguntar when the current video's Ask panel can be opened or is already expanded within the configured wait window. The system MUST use Chapters/Capítulos only when Ask is not available within that wait window. The system MUST treat only a confirmed Chapters/Capítulos panel with visible chapter items as a valid chapters candidate, and MUST treat ambiguous `In this video` / Timeline / Transcript surfaces as noisy panels rather than valid Chapters.

#### Scenario: Ask panel is available and can be opened

- **WHEN** a supported YouTube watch page for the current confirmed video loads
- **AND** the current video's `Ask` panel exists with `visibility="ENGAGEMENT_PANEL_VISIBILITY_HIDDEN"` or otherwise exposes a visible Ask/Preguntar control within the configured wait window
...
#### Scenario: Ask and Chapters are both available

- **WHEN** a supported YouTube watch page for the current confirmed video loads
- **AND** the current video's `Ask` panel can be opened or is already expanded within the configured wait window
- **AND** the current video's `Chapters`/`Capítulos` engagement panel also exists with visible chapter items
- **THEN** the system MUST choose Ask for the automatic opening decision
- **AND** the system MUST NOT open the chapters panel for that automatic opening decision

#### Scenario: Chapters panel is available after Ask is unavailable

- **WHEN** a supported YouTube watch page for the current confirmed video loads
- **AND** no current-video Ask panel or Ask/Preguntar control can be opened or confirmed within the configured wait window
- **AND** the current video's `Chapters`/`Capítulos` engagement panel exists or becomes available
- **AND** visible chapter items (`ytd-macro-markers-list-item-renderer`) can be confirmed
...
- **AND** no current-video Ask panel or Ask/Preguntar control is available within the configured wait window
- **THEN** the system MUST NOT treat that surface as a valid chapters panel
- **AND** the system MUST continue silently when no valid fallback panel is available

#### Scenario: Current video's Ask panel is already open

- **WHEN** a supported YouTube watch page for the current confirmed video loads
- **AND** the current video's `Ask` panel is already expanded within the initial decision window
...
#### Scenario: Chapters is already open but Ask becomes available during the initial decision

- **WHEN** a supported YouTube watch page for the current confirmed video loads
- **AND** the current video's `Chapters`/`Capítulos` engagement panel is already expanded with visible chapter items
- **AND** the current video's Ask panel can be opened or is already expanded within the configured wait window
- **THEN** the system MUST choose Ask for the automatic opening decision
- **AND** the system MUST mark the current video complete after handling Ask

#### Scenario: Ask appears after Chapters fallback completed

- **WHEN** a supported YouTube watch page for the current confirmed video loads
- **AND** no current-video Ask panel or Ask/Preguntar control is available within the configured wait window
- **AND** the system opens Chapters as the fallback and marks the current video complete
- **AND** Ask becomes available later for the same video without a manual user open
...
- **WHEN** the system opens Ask for the current confirmed video
...
- **AND** the system MUST NOT fall back to Chapters for that video
- **AND** the system MUST mark the current video complete
