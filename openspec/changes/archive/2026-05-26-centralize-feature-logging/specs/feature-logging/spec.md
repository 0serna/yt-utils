...
The system SHALL persist feature log entries in `chrome.storage.local` under the key `yt-utils:logs` as a chronological JSON array.
...
The system SHALL retain only the latest 1000 feature log entries in `yt-utils:logs`.
