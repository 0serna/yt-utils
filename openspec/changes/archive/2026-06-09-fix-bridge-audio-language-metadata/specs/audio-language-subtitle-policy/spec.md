...
#### Scenario: Active audio track reports Spanish in US metadata shape

- **WHEN** the current confirmed watch page's active player audio track reports Spanish metadata through the `US` YouTube audio-track metadata shape
- **THEN** the extension treats the video as non-English-audio content for subtitle policy decisions

#### Scenario: Active audio track has opaque top-level ID and usable metadata ID

- **WHEN** the current confirmed watch page's active player audio track exposes an opaque top-level ID
- **AND** the active audio track exposes a usable language code through a recognized metadata shape
- **THEN** the extension uses the recognized metadata language code instead of the opaque top-level ID

#### Scenario: Active audio track has only opaque IDs

- **WHEN** the current confirmed watch page's active player audio track exposes only opaque or non-language IDs
- **AND** the audio-track name does not identify English or Spanish
- **AND** the player does not expose a direct English auto-generated caption track
- **THEN** the extension treats the audio language as unknown and does not expose the opaque ID as active audio language

#### Scenario: Active audio track has opaque ID but Spanish name

- **WHEN** the current confirmed watch page's active player audio track exposes only opaque or non-language IDs
- **AND** the active audio-track name identifies Spanish audio
- **THEN** the extension treats the video as non-English-audio content for subtitle policy decisions
