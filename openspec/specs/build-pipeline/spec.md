...
The project SHALL use TypeScript with `strict: true` in `tsconfig.json`. All source files in `src/` SHALL pass the repository validation commands without errors.

#### Scenario: Validation passes

- **WHEN** a contributor runs `npm run lint` and `npm run typecheck`
- **THEN** ESLint reports no lint issues and the TypeScript compiler reports no type errors across all source files in `src/`
